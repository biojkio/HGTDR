...
# 0. 路径配置
# ─────────────────────────────────────────────────────────────────────────────
DATA_ROOT   = '/kaggle/input/datasets/jkiobio/hgtdr-data-simple/data'
    ...
    for entity_id, hgt_id in mapping.items():
        if node_type == 'drug':
            if entity_id in smiles_dict:
                data[node_type].x[hgt_id, :] = torch.tensor(
...
def make_fold_source_data(val_edge_index: torch.Tensor) -> HeteroData:
    ...
    keep = torch.tensor(
        [(s.item(), d.item()) not in val_set
         for s, d in zip(full_ei[0], full_ei[1])],
        dtype=torch.bool
    )
    src[(node_type1, rel, node_type2)].edge_index = full_ei[:, keep]
...
print('\n训练代码修改提示:')
print('  将 train1.pkl / val1.pkl 替换为 train_mp1.pkl / val_mp1.pkl')
print('  删除训练代码中的 AddMetaPaths 调用（元路径已预先注入）')
print('  删除训练代码中的 add_mask 调用（mask 已预先生成）')