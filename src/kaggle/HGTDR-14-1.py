...
def define_loaders(config):
    kwargs = {'batch_size': config['batch_size'], 'num_workers': 2, 'persistent_workers': True}
...
def make_test_batch(batch):
    ...
    while len(neg_edges_source) < pos_num:
        ...
        orig_src = batch[node_type1]['id'][source_node].item()
        orig_dst = batch[node_type2]['id'][dest_node].item()
        if (orig_src, orig_dst) not in global_pos_set:  # ← O(1)
...
def train(GNN, pred, model, loader, optimizer):
    ...
    for i, batch in enumerate(iter(loader)):
        ...
        edge_label_index = batch[(node_type1, rel, node_type2)]['edge_label_index']
        edge_label = batch[(node_type1, rel, node_type2)]['edge_label'].to(device)
...
def run(config):
    ...
    no_improve_count = 0          # 连续未改善的 epoch 计数器
    # ─────────────────────────────────────────────────────────
        ...
        if val_loss < best_val_loss - min_delta:
            ...
            if no_improve_count >= patience:
                ...
                print(f"\n[早停] 在 epoch {epoch} 触发早停。"
                      f"最佳验证损失 {best_val_loss:.4f} 出现在 epoch {best_epoch}。")
                break
        # ─────────────────────────────────────────────────────────
