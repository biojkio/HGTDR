# %% [markdown]
# define_loadersnum_workers=2（修正），新增 pin_memory=True
# make_batchwhile循环 → torch.randint 批量采样再过滤，消除CPU瓶颈
# make_test_batch同上，负采样向量化
# train / test.to(device) → .to(device, non_blocking=True)，异步传输
# define_model末尾加 torch.compile，第1个epoch慢（编译期），之后每轮提速10~30%
...
def define_model(dropout):
    ...
    # torch.compile：第一个 epoch 会额外花时间编译，之后每轮快 10~30%
    # 若 PyTorch 版本 < 2.0 或环境不支持，可注释掉这两行
